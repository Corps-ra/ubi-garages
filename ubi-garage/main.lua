...
function SpawnVehicle(vehicle, plate,fuel, vector, heading, netId, svData)
	...
	exports['mythic_notify']:SendAlert('inform', 'Tidak ada spot untuk kendraan')
	end
end

-- TriggerServerEvent("ubi-garage:setvehicelhpim", "2")
...
RegisterNetEvent("advancedGarage:OpenCarGaragesherriff")
AddEventHandler("advancedGarage:OpenCarGaragesherriff", function()
OpenPoliceSheriffGarageMenu()
end)

function OpenPoliceSheriffGarageMenu()
	local cars = {}
	local i = 1
	local wait = true
	
		ESX.TriggerServerCallback('ubi-garage:policegarageSandy', function(ownedPoliceCars)
			local menu = {}
	...
	if IsPedInAnyVehicle(playerPed,  false) then
		...
		ESX.TriggerServerCallback('ubi-garage:storeVehicle', function(valid)
			if valid then
				...
				exports['mythic_notify']:SendAlert('error', 'Tidak bisa menyimpan kendaraan')
			end
		end, vehicleProps)
	else
		exports['mythic_notify']:SendAlert('error', 'Tidak ada kendaraan')
	end
end
-- End of Police Code
	...
	ESX.TriggerServerCallback('ubi-garage:getOwnedVehicles', function(ownedCars)
        ...
        if ownedCars ~= nil then
            ...
            if #menu ~= 0 then
				...
				exports['mythic_notify']:SendAlert('error', "Kamu tidak bisa memarkirkan kedaraan di sini")
            end
        end
	end)
end
...
RegisterNetEvent("ininponund:Garage", function()
	exports['mythic_notify']:SendAlert('error', 'Kendaraan sedang di luar garasi!')
end)
--end location
...
if ESX.Game.IsSpawnPointClear(vector3(GetEntityCoords(ped)-1), 2.0) then	   
	...
	exports['mythic_notify']:SendAlert('error', "Tidak ada spot parkir")
   end
end)
...
AddEventHandler('admin:car', function(vehicleProps, plate, fuel, modelveh)
	...
	ESX.TriggerServerCallback('ubi-garage:getvehown', function(owened)
		if owened == true then
			exports['mythic_notify']:SendAlert('error', 'Kendaraan sudah ada di garagsi!')
			return
		else
			if vehicleProps.bodyHealth == 1000.0 then
				vehicleProps.bodyHealth = 999.9
			...
			exports['mythic_notify']:SendAlert('inform', 'kendaraan di simpan')
			end
		end, vehicleProps)
	else
		exports['mythic_notify']:SendAlert('error', 'tidak ada kendaraan untuk di simpan')
		return
	end
		else 
			exports['mythic_notify']:SendAlert('error', 'Kamu bukan admin!')
		end
	end)
end) 
...
function StoreVehicle(vehicle, vehicleProps, location, data)
	...
	exports['mythic_notify']:SendAlert('inform', 'Kendaraan disimpan di garasi : ' ..currentLoc)
end
